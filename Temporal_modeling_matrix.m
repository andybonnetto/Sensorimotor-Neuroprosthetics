function [M_cells,V_0] = Temporal_modeling_matrix(cell_list,init,L,pfile)
    ...
    ncols = Constants.t_size;
    outputstr = ['%' num2str(mrows) 'f']; % template for the string, you put your datatype here
%     outputstr = repmat(outputstr, 1, ncols); % replicate it to match the number of columns
    ...
    for t = [2:Constants.t_size]
        t;
        if nargin > 2
            M_cells = get_Gsyn(M_cells,L);
        else
            M_cells = get_Gsyn(M_cells);
        end
        M_cells = update_V_m(M_cells,t);
        fprintf(fid,outputstr, M_cells.V_m.');
        S = get_firing_rate(M_cells);
    end
    fclose(fid);
...
function M_cells = get_Gsyn(M_cells,L)
    M_cells = M_cells.update_V_pre();
...
function M_cells = update_V_m(M_cells,t)
    ...
    M_cells.V_m(:) = (- repmat(Constants.tau_syn,M_cells.N_cells,1)./tau_m).*M_cells.V_m(:) + (repmat(Constants.tau_syn,M_cells.N_cells,1)./tau_m).*V_s;
...
function S = get_firing_rate(M_cells)
    ...
    for name = names 
        ...
        DV_t = max(zeros(sum(ganglions),1),M_cells.V_m(ganglions)- E_T);
        n  = 2 + (0.1*DV_t).^2;
