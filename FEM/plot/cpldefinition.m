function cpltag = cpldefinition(model, type, varargin )
...
cpltag = char( model.result.dataset.uniquetag('cpl') );
...
switch type
    case 'xy' % Plane defined by 1 variable : the height z 
        ...
        model.result.dataset(cpltag).set('quickz', height);
        %model.result.dataset(cpltag).set('spacevars', {'cpl3x' 'cpl3y'});
        %model.result.dataset(cpltag).set('normal', {'cpl3nx' 'cpl3ny' 'cpl3nz'});
    
    case 'yz' % Plane defined by 1 variable : the depth x 
        depth = varargin{1};
        % model.result.dataset(cpltag).set('quickz', [append(height,'[') native2unicode(hex2dec({'00' 'b5'}), 'unicode') 'm]']);
        model.result.dataset(cpltag).set('quickplane', 'yz');
        model.result.dataset(cpltag).set('quickx', depth);
        %model.result.dataset(cpltag).set('spacevars', {'cpl3x' 'cpl3y'});
        %model.result.dataset(cpltag).set('normal', {'cpl3nx' 'cpl3ny' 'cpl3nz'});
