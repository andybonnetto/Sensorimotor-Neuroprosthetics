...
for name = names
    ...
    plot(linspace(0,Constants.simulation_duration,Constants.t_size+1), M_cells_h.V_m(d,:)*1e3)
    title(name)
    xlabel("Time [s]")
    ylabel("Voltage [mV]")
    k = k +1;
...
S = get_firing_rate(M_cells_h,1,folder_path, new_thresholds,[]);
...
for name = names
    subplot(3,3,k)
    d = M_cells.names == name;
    d2 = vecnorm([mat3D(:,1),mat3D(:,2)].') < 300e-6;
    d3 = d&d2;
    plot(linspace(0,Constants.simulation_duration,Constants.t_size+1), M_cells.V_m(d3,:)*1e3)
    title(name)
    xlabel("Time [s]")
    ylabel("Voltage [mV]")
    k = k +1;
